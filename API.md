...
## Agent Steering (removed)

The steering module (`40mcp/steering` — forced-inference write classification,
prehook/posthook instructions, authority tiers, `AgenticMemory`) was removed
pre-1.0. It was orthogonal to the bridge and widened the public surface without
a consuming use case. The `_steering` envelope key remains reserved and is
stripped on every egress path, so neither operators nor upstreams can shadow
it. Configs that still declare `tool.steering`, pass `--steering`, or set
`settings.frontdoor.steering` fail validation with a removal message.
